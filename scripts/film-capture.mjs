#!/usr/bin/env node
/**
 * film-capture.mjs: pose-and-photograph capture of film/index.html over the Chrome DevTools Protocol.
 *
 ...
 *
 * Adapted from the zero-dependency motion-film capture (Node 22 ships WebSocket and fetch).
 *
 *   node scripts/film-capture.mjs --theme dark --cut film --fps 60 --out /tmp/acs-film/dark
 *   node scripts/film-capture.mjs --theme light --cut loop --fps 1 --out /tmp/acs-film/review
 *   node scripts/film-capture.mjs --theme dark --times 0,3,7.5 --out /tmp/acs-film/stills
 */
...
async function worker(o, times, names, label) {
  ...
  const userDataDir = join('/tmp', `acs-film-chrome-${process.pid}-${label}`)
...
async function durationOf(o) {
  const probe = { ...o, out: join('/tmp', `acs-film-probe-${process.pid}`) }
  mkdirSync(probe.out, { recursive: true })
  // Reuse the worker to load the page once and read its duration.
  const port = 9300 + Math.floor(Math.random() * 600)
  const userDataDir = join('/tmp', `acs-film-chrome-${process.pid}-probe`)
  const chrome = launchChrome(port, userDataDir)
  let cdp
  try {
    cdp = connect(await waitForDevTools(port))
