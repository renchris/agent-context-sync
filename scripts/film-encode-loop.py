#!/usr/bin/env python3
"""film-encode-loop.py: encode the README loop's frames into one animated WebP.

    python3 scripts/film-encode-loop.py FRAMES_DIR OUT.webp [--hold nl|q<N>] [--flight q<N>]
...
spans where the camera moves; film-capture.mjs --flight-fps samples those at 20 fps instead of 30.
...
film-verify.py can check each stored frame against its own source, the way it was encoded.
"""
