<!-- Hero loop: generated by `npm run hero` (scripts/render-hero.mjs); direction in docs/design/hero/DIRECTION.md. -->
<picture>
  <source media="(prefers-color-scheme: dark)" srcset="docs/media/hero-dark.svg">
  <img src="docs/media/hero-light.svg" width="100%" alt="Keep an agent-readable docs/ folder in sync with OneDrive, SharePoint, Outlook and Teams by processing only what changed. Four steps: 1, ask each source what changed, and make expiry cheap; 2, decide with three hashes before reading a byte; 3, read bytes only on purpose; 4, publish a pure function of the source, in git. The picture opens on the outcome of one sync: 2 of 256 source objects read, and one commit in docs/. It then replays how the sync got there, over 256 objects in four blocks, one per source. Eight objects are reported. SharePoint's since-token expires (410), so its metadata is walked instead, reading 0 bytes. Six of the eight are equal at H0 and cost 0 bytes. The other two are read, and those reads are the only amber on screen. One of them is a no-op save whose page is unchanged at H2, and the other becomes one commit in docs/.">
</picture>
