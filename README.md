...
<div>
<a href="https://github.com/user-attachments/assets/599dccd5-ac9d-4d82-afce-9554282048c2"><picture>
  <source media="(prefers-color-scheme: dark)" srcset="docs/media/hero-dark.webp">
  <img src="docs/media/hero-light.webp" width="100%" alt="Keep an agent-readable docs/ folder in sync with OneDrive, SharePoint, Outlook and Teams by processing only what changed. Four steps: 1, ask each source what changed, and make expiry cheap; 2, decide with three hashes before reading a byte; 3, read bytes only on purpose; 4, publish a pure function of the source, in git. The picture: a company's files as four lanes of documents running to the horizon, one lane per source. Reading bytes is the only thing on that plane with height. After one sync, two equal amber pillars stand up into the sky beside the words Two files read. One commit. The loop then cuts to a high view of all four lanes. Each source reports the files that changed, eight in all; six are decided equal before a byte is read and leave the field. The other two rise slowly into the pillars. One of them, a spreadsheet saved without real change, gets a green cap and stops there. From the other, a converted page lifts off, descends, and comes to rest beside the words One commit.">
</picture></a>
<br>
<sub><a href="https://github.com/user-attachments/assets/599dccd5-ac9d-4d82-afce-9554282048c2">▶ Watch the 25-second launch film</a> · a design with measured probes, not yet an implementation</sub>
</div>
