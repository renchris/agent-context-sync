...
(`user@example.com`, `driveType: personal`) — labelled CONSUMER-PROBE; ODB/SPO differs in two
...
```
"eTag": "\"{9B8A7C6D-1E2F-4A3B-8C4D-5E6F7A8B9C0D},4\""
"cTag": "\"c:{9B8A7C6D-1E2F-4A3B-8C4D-5E6F7A8B9C0D},2\""
"file": { "mimeType": "application/pdf",
          "hashes": { "quickXorHash": "EXAMPLEqXorHashValue0000002=" } }
```
...
- Delta's remark "The `parentReference` property on items won't include a value for **path**" was
  **false** in the observed response — `"path": "/drives/0123456789ABCDEF/root:"` was present. Do not
...
| **`Receipt 3.pdf`** | **`receipt 3`** | **—** | — | **no sibling `Receipt.pdf` exists ⇒ the 3 is CONTENT, not a version.** Real case from the probed drive: `Receipt 3.pdf` (1,051,760 B) and `Receipt 4.pdf` (1,055,899 B) are two different receipts. |
...
source:
  ...
  guid:     9B8A7C6D-…        # the GUID inside eTag/cTag; second witness to identity
  path:     /Shared Documents/Clients/Acme/Report v2.xlsx
  name:     Report v2.xlsx
  ctag:     'c:{9B8A7C6D-…},2'   # content counter — re-convert iff this moves
  etag:     '{9B8A7C6D-…},4'     # metadata counter — path-only update iff only this moves
  quick_xor: EXAMPLEqXorHashValue0000002=
  size:     1055899
...
|---|---|---|---|
| 1 | **Two different documents share a stem** (`Receipt 3/4.pdf`, observed live) | trailing integer with **no** un-suffixed sibling; text_sha differs; containment low | Do not strip the integer — it is content. Separate families. |
...
- **Order families by filename version tokens.** Ruled out by failure 2 and by the `Receipt`
  counterexample.
