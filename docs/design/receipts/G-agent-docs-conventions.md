...
2000-line Read default**, and the research repo's own docs already breach it — one research file is 146,511
...
| The research repo's real docs: measured | 1,192 `.md` files, 35 MB. Largest: one research report = 146,511 chars / 1,982 lines / 20,707 words. Mean 61–73 chars/line across 5 sampled files. | `wc` on the research repo's `docs/` |
...
**The local `repo-wiki` skill** (a private Claude Code skill,
not published) already encodes the decomposition and citation
...
| **date in the filename** (`-YYYY-MM-DD` suffix) | **WORKS.** Survives clone, greppable, sortable by `ls`, and visible in a Glob listing without a Read. The research repo already does it (e.g. `agent-context-sync-2026-09-21.md`). | measured on the local corpus |
...
the only tool here that sees an empty field. (This is a known failure in the research repo's own corpus —
a prior local lesson.)
...
7. **Converter determinism is untested here and is a silent corpus-wide hazard.** If the PDF/DOCX
   ...
   the same file twice and require byte-identical output. (Same class as the research repo's
   prior local lesson, where a PNG `tIME` chunk made identical renders
...
| repo-wiki structure pass | page plan table with **files this page rests on** — already the dependency map | local skill | written for one-shot generation; nothing persists the table | L4 map | local repo-wiki skill, plan table |
