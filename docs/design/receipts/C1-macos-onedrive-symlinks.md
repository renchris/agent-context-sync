...
- **`grep -R` does not follow on macOS, and the man page says so.** `/usr/bin/grep --version` →
  ...
  on recursive grep and symlinks,
...
- **`find` on a symlinked start dir returns ZERO, not an error.** Exactly the failure recorded in
  a second local lesson (a symlinked store directory, 0 vs 123 files). Reproduced verbatim
...
- **`git ls-files -s` → `120000 <blob-oid> 0 docs-source`.** Mode `120000` is a
...
- **Claude Code's tools have no escape hatch.** `@anthropic-ai/claude-code` 2.1.114 `sdk-tools.d.ts:413-483`:
...
- Theoretical, high confidence: **for items with no local node (P7), no** — there is no inode to emit an
  ...
  That is the alarm-polarity failure the research repo already has a rule for.
...
- **Trap, theoretical and near-certain: FSEvents reports canonical paths.** A watcher started on
  `/repo/docs-source` (a symlink) receives paths under `~/Library/CloudStorage/...`. A consumer that
