...
Claim: "Live counterexample: `Receipt 3.pdf` and `Receipt 4.pdf` are different documents, and their `createdDateTime` order is the *inverse* of their client-supplied modified order."
Problem: H records two separate live facts — the receipt pair are different receipts (H:167, :354) and "two items whose `createdDateTime` order is the inverse of their `fileSystemInfo.lastModifiedDateTime` order" (H:355) — and never says the inverted-clock items are the two receipts. The document welds them into one observation.
Fix: "Live counterexamples: `Receipt 3.pdf` and `Receipt 4.pdf` are different documents; separately, two probed items had a `createdDateTime` order that is the *inverse* of their client-supplied modified order."
