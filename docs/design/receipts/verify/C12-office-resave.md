...
**Result: not measurable on this machine. Office is unactivated, so it cannot save.** Two operator-driven runs of `agentsync-office-resave.sh` (durable copy `probes/agentsync-office-resave.sh`) captured no save in any app: every round timed out with the target file's mtime and inode unchanged and no new file anywhere a Save As could land.
...
Once the operator activated Office with the tenant's Microsoft 365 Business Standard licence (user@contoso.com; a new licence file appeared under `Licenses/5/` at 23:00), every save the §2 table recorded as `-50` / `-1708` succeeded by AppleScript. That confirms §5: the gate was the licence, not scripting. Script: `/tmp/agentsync-office-resave-auto.sh` (Office 16.x on macOS; files opened with `open -a`, so no "Grant File Access" prompt). For each format, fixture `a.*` → working copy → Office save (**b**) → Office save again with no net content change (**c**) → compare b vs c. Office wrote both sides.
