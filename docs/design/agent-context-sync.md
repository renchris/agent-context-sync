...
> **Published copy.** Receipt paths in this document (`verify/…`, `review/…`, the axis reports) resolve under [`receipts/`](receipts/). Account names, tenant ids, file names and local paths from the original run were replaced with placeholders (Contoso, `user@example.com`, `~/`); every measurement is unchanged. There is no implementation yet: §10 is the build order.

*2026-09-21. Research wave of 14 axes (12 productive, 2 adversarial) plus 10 adversarial verifiers on the load-bearing claims. Per-axis reports and verifier reports are in [`receipts/`](receipts/). Everything marked **measured** was run on this Mac or against a live Graph tenant during the wave; everything marked **documented** is quoted from a vendor reference; the rest is reasoning and says so.*
...
All measured on this Mac (macOS 15.7.9, rg 15.2.0, `/usr/bin/grep` 2.6.0-FreeBSD). Two prior local lessons record the same failure (recursive grep and `find` both blind to a symlinked tree).
...
|---|---|---|
| `.docx` | **pandoc 3.11** `-f docx -t gfm --extract-media`; Docling for image-heavy or span-heavy | measured 2026-09-22 (`verify/C13`): byte-identical across two runs; header row kept as `<thead>`; a **merged cell makes pandoc emit the whole table as HTML with `colspan`** (GFM pipe tables cannot express spans) — greppable, not a pipe table; MarkItDown emits a blank header row (measured) |
...
- **Store citations as path + content hash, with line ranges display-only**: line-ranged citations rot on any insertion (a prior local lesson, and DeepWiki's citation form).
...
- **Page budget ≤ 400 lines / ≤ 25 KB / ~6 K tokens, hard split at 800 lines.** The binding constraint is tokens, not the Read tool's 2,000-line default (≈ 34 K tokens at the research repo's measured 61–73 chars/line; one existing research file here is 146 K chars). 25 KB is Anthropic's own shipped budget for a file meant to be read whole.
...
| cost per cycle | 1 RU per drive (0.12 % of the smallest per-app budget at 60 s) | one `getattrlistbulk` walk, zero opens, zero downloads — **0.13–0.31 s per 10⁵ files on local APFS (measured, `verify/C11-local-walk.md`)**; File Provider measured the same as local APFS on already-enumerated directories (2,000 files in 0.002–0.004 s, `verify/C14` §3); a never-enumerated directory is a provider round-trip |
...
None of these could be measured in the first wave (no account was signed in). **Status 2026-09-23:** probes 3, 4, 5 and 6 are now MEASURED on a Business Standard tenant (rows below); 1, 2, 4b, 8 and 9 remain, and they need the target corporate tenant, because consent policy, library hash coverage and token lifetime are properties of that tenant. Each is one probe, and each decides an arm:
...
| 3 | With a since-token watcher on `~/Library/CloudStorage/…` and another on the OneDrive private store, edit a file from the web UI: which fires, with which flags, for (i) a materialized file, (ii) a dataless file, (iii) a never-browsed subtree **MEASURED 2026-09-23 (`verify/C14` §4):** (i) a web edit to a downloaded file raised `Modified,ChangeOwner,XattrMod` on the canonical `CloudStorage` path within ~20 s and updated the copy in place; (iii) a folder + file created on the web raised only the *directory* event on the canonical path — the child surfaced (dataless, `gen=1`) only when the folder was enumerated, so T1 on a new directory must trigger a T3 walk of it; (ii) not measured (the test file never appeared in the web listing, §5 there). The OneDrive private store mirrors the tree with richer events but is undocumented state — do not depend on it. | whether arm B's T1/T2 exist at all on File Provider |
| 4 | `ls -l% <online-only file>` and `stat -f '%f %z %b'` (expect `%`, full size, 0 blocks, bit 0x40000000 set); then `cat` it under the default policy (expect: blocks, downloads) and under a `setiopolicy_np` materialize-OFF wrapper (expect `EDEADLK`, errno **11** on macOS — the "errno 35" in claude-code #40783 is Linux's number, from that issue's Ubuntu-VM sandbox); then check the same from a launchd agent, whose per-job `MaterializeDatalessFiles` key may differ from the login session **MEASURED 2026-09-23 (`verify/C14` §2):** all four predictions held in the login shell (`%`, full size, 0 blocks, `0x40000000`; default read downloads; materialize-OFF read fails `EDEADLK` errno 11 — at `read()`, `open()` succeeds). **From a launchd job the default policy is OFF**, so a plain read fails errno 11 there; `setiopolicy_np(…_ON)` or plist `MaterializeDatalessFiles=true` restores downloading. Right after sign-in a dataless read failed `ETIMEDOUT` (errno 60) twice before succeeding. | the hydration model every stage assumes, in both contexts the pipeline runs in |
...
| 5 | `getattrlistbulk` walk of the pinned subtree: is `ATTR_CMN_GEN_COUNT` in `RETURNED_ATTRS` on File Provider, and what is the wall-clock cost there? **Local APFS floor now measured (2026-09-22, receipt `verify/C11-local-walk.md`):** a 60-line `getattrlistbulk` walker over 100,000 files in 1,001 directories took 0.31 s on the first pass after creation and 0.13–0.14 s warm; a Python `os.scandir`+`lstat` walk took 0.30 s. T3 alone is fast enough on local disk to run at every session start with no watcher; only the File Provider arm remains unmeasured **File Provider MEASURED 2026-09-23 (`verify/C14` §3):** `ATTR_CMN_GEN_COUNT` is returned for every item (2,000/2,000, dataless included); a 2,000-file walk took 0.002–0.004 s on OneDrive vs 0.002 s on local APFS, so T3 alone is fast enough there too. | whether T3 alone is fast enough to skip the watcher — locally, yes |
| 6 | Open a real tenant `.xlsx`/`.docx`/`.pptx` in Excel/Word/PowerPoint, Save without edits, diff the per-part rollup excluding `docProps/*`. **Attempted here 2026-09-22 and GUI-gated (receipt `verify/C12-office-resave.md`):** scripted Office (AppleScript, files inside each app's sandbox container) never completed a save — Excel returned `-50` on every save form, Word `-1708`, PowerPoint sat on a modal an unattended session cannot answer — so the Office arm needs one manual Save per app on a real file, then the comparator. **Re-diagnosed 2026-09-23 (`verify/C12` §5): not GUI-gated but LICENCE-gated** — Office here runs unactivated ("View Only — Activate Microsoft 365 to Create and Edit"), Save and Save As are disabled in the menu on a modified workbook, and the operator-driven run captured nothing in any app; the Office arm needs an activated Office. **MEASURED 2026-09-23 (`verify/C12` §6), once Office was activated:** after a no-edit Office re-save, the parts outside `docProps/*` were identical for `.pptx`; `.xlsx` differed only in `xl/workbook.xml` (`xr:revisionPtr/@documentId`, a per-save GUID); `.docx` differed in `word/document.xml` and `word/settings.xml` (revision-save ids `w:rsid*` / `<w:rsids>`). The converter output (H2) was identical for all three. **LibreOffice's headless round-trip did run:** a no-op re-save changed `xl/styles.xml` and both `xl/worksheets/sheetN.xml` parts, so the volatile-part denylist is NOT sufficient across writers and only the output-hash early cutoff (H2) makes a no-op save free | how narrow H1-on-parts is — for LibreOffice-written files it is not narrow at all; H2 is load-bearing, not an optimization |
| 7 | Convert a 50-file sample twice with each chosen converter, compare bytes — pandoc 3.11 now measured here: byte-identical ×2 on docx (`verify/C13`); MarkItDown and PyMuPDF4LLM ×3 in the E report | the double-conversion gate's baseline |
